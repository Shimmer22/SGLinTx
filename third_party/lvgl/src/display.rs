...
impl<'a> Display {
    ...
    {
        // NOTE:
        // LVGL keeps raw pointers to `disp_drv` and to draw buffers after registration.
        // If `DisplayDriver` is stack-allocated here, it gets dropped at function return and
        // all those pointers become dangling. Keep it alive for process lifetime by leaking it.
        let mut display_diver = DriverBox::new(DisplayDriver::new(draw_buffer, display_update)?);
        display_diver.init_draw_buf_ptr()?;
        ...
        let leaked = DriverBox::into_raw(display_diver);
        let leaked = unsafe { &mut *leaked };
        Ok(disp_drv_register(leaked, None)?)
    }
    ...
    pub unsafe fn register_raw<const N: usize>(
        ...
        // NOTE: see `register` above. Keep the driver allocation alive for process lifetime.
        let mut display_driver = DriverBox::new(DisplayDriver::new_raw(
            draw_buffer,
            flush_cb,
            rounder_cb,
            set_px_cb,
        ...
        display_driver.init_draw_buf_ptr()?;
        let disp_p = &mut display_driver.disp_drv;
        disp_p.hor_res = hor_res.try_into().unwrap_or(240);
        disp_p.ver_res = ver_res.try_into().unwrap_or(240);
        let leaked = DriverBox::into_raw(display_driver);
        let leaked = &mut *leaked;
        Ok(disp_drv_register(leaked, drop)?)
    }
}
...
impl<'a, const N: usize> DisplayDriver<N> {
    fn init_draw_buf_ptr(&mut self) -> Result<()> {
        self.disp_drv.draw_buf = Box::<_lv_disp_draw_buf_t>::into_raw(
            self._buffer
                .get_ptr()
